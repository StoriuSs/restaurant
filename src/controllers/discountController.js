...
class DiscountController {
	...
	async getAllDiscounts(req, res) {
		try {
			const [rows] = await db.execute(
				`SELECT * FROM Discount ORDER BY discount_id`
			);
			// Format date fields to yyyy-mm-dd
			const formattedRows = rows.map((d) => ({
				...d,
				valid_from: d.valid_from
					? d.valid_from.toISOString().split("T")[0]
					: null,
				valid_to: d.valid_to
					? d.valid_to.toISOString().split("T")[0]
					: null,
			}));
	...
	async deleteDiscount(req, res) {
		const { id } = req.params;
		try {
			await db.execute(`DELETE FROM Discount WHERE discount_id = ?`, [
				id,
			]);
			res.json({ success: true });
		} catch (error) {
